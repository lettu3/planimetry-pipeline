...
export async function fetchPlayCanvas(){
    try{
      console.log(
        `🔎 Buscando escenas para el proyecto ${config.playcanvas.project_id}...`,
      );

      const scenesRes = await fetch(
        `https://playcanvas.com/api/projects/${config.playcanvas.project_id}/scenes`,
        { headers },
      );
      if (!scenesRes.ok)
        throw new Error(`Error HTTP escenas: ${scenesRes.status}`);

      // Aplicamos la interfaz PlayCanvasSceneListResponse
      const scenesData =
        (await scenesRes.json()) as PlayCanvasSceneListResponse;
      ...
      const mainScene = scenesData.result.find(
        (s: any) => s.name.toLowerCase() === "main",
      );
      ...
      if (mainScene) {
        console.log(
          `✅ Escena principal (Entrypoint) detectada: "${mainScene.name}" (ID: ${mainScene.id})`,
        );

        // Filtramos las demás escenas para no duplicar la principal
        const otherScenes = scenesData.result.filter(
          (s: any) => s.id !== mainScene.id,
        );
        ...
        console.warn(
          `⚠️ No se encontró una escena llamada "Main". Usando la primera devuelta por la API como principal.`,
        );
      ...
      const branchesRes = await fetch(
        `https://playcanvas.com/api/projects/${config.playcanvas.project_id}/branches`,
        { headers },
      );
      if (!branchesRes.ok)
        throw new Error(`Error HTTP branches: ${branchesRes.status}`);

      const branchesData =
        (await branchesRes.json()) as PlayCanvasBranchListResponse;
      if (!branchesData.result || branchesData.result.length === 0) {
        throw new Error("No se encontraron ramas (branches) en el proyecto.");
      }

      // se busca la branch puesta en config
      let targetBranch = branchesData.result.find(
        (b: any) =>
          b.name.toLowerCase() === config.playcanvas.target_branch_name ||
      ...
      if (!targetBranch) {
        console.warn(
          "⚠️ No se encontró la rama 'main' o 'master'. Las ramas disponibles son:",
        );
        branchesData.result.forEach((b: any) => console.log(`   - ${b.name}`));
        throw new Error(
          "Abortando: No hay rama principal disponible para compilar.",
        );
      }

      const targetBranchId = targetBranch.id;
      console.log(
        `✅ Branch detectada: "${targetBranch.name}" (ID: ${targetBranchId})`,
      );
      // ------------------------------------------------
      ...
      let downloadUrl = "";
      while (true) {
        const jobRes = await fetch(`https://playcanvas.com/api/jobs/${jobId}`, {
          headers,
        });
        ...
        if (jobData.status === "complete") {
          ...
          const detalles =
            jobData.messages && jobData.messages.length > 0
              ? jobData.messages.join("\n - ")
              : "Sin detalles adicionales del servidor.";

          throw new Error(
            `El build falló internamente en PlayCanvas.\nDetalles del error:\n - ${detalles}`,
          );
        }
